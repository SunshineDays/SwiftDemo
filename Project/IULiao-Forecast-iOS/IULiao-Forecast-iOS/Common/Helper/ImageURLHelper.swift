...
class ImageURLHelper {
    ...
    /// 尺寸
    @discardableResult
    func size(w: Int, h: Int) -> ImageURLHelper {
    ...
    /// 尺寸
    func size(_ size: CGSize) -> ImageURLHelper {
        return self.size(w: Int(size.width), h: Int(size.height))
    }
    
    /// 裁剪
    func chop(mode: CropModeType = .alwayCrop) -> ImageURLHelper {
        params.updateValue("\(mode.rawValue)", forKey: "crop")
        return self
    }
    
    /// 质量
    func quality(number: Int = 75) -> ImageURLHelper {
        params.updateValue("\(number)", forKey: "q")
        return self
    }
    
    /// 图片格式
    func format(ofType: String = "jpg") -> ImageURLHelper {
        params.updateValue(ofType, forKey: "format")
        return self
    }
    
    /// 原始大小
    func originSize() -> ImageURLHelper {
        params = [:]
        return self
    }
