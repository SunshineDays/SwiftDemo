...
class UserInfoHandler: BaseHandler {
    ...
    func uploadAvatar(avatarImage: UIImage?, success: @escaping ((UserInfoModel) -> Void), faild:@escaping FailedBlock){
        ...
        Alamofire.upload(
            multipartFormData: { (form) in
                let data = UIImage.jpegData(image)(compressionQuality: 1)
            ...
            case let .success(request,_,_):
                request.responseJSON(completionHandler: { (res) in
                    let json = try! JSON(data:res.data ?? Data())
    ...
    /// 用户详情
    func detail(success: @escaping ((UserInfoTotalModel) -> Void), failed: @escaping FailedBlock) {
        ...
        defaultRequestManager.requestWithRouter(
            ...
            success: {
                (json) in
                let userInfoTotalModel = UserInfoTotalModel(json: json)
                success(userInfoTotalModel)
                
                UserToken.shared.update(userInfo: userInfoTotalModel.user)
        },
            failed: {
                (error) -> Bool in
                failed(error)
