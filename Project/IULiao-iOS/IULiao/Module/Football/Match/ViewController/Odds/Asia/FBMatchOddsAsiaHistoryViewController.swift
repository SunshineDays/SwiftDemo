...
/// 赛事分析 指数 亚盘 详细(历史)
class FBMatchOddsAsiaHistoryViewController: TSEmptyViewController, TSNestInnerScrollViewProtocol, FBMatchViewControllerProtocol, FBMatchOddsCompanyProtocol {
    ...
    private var oddsList = [FBOddsAsiaModel]()
...
extension FBMatchOddsAsiaHistoryViewController {
...
extension FBMatchOddsAsiaHistoryViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let prevAsia = oddsList[safe: row + 1] ?? asia
        cell.configCell(matchInfo: matchInfo, asia: asia, prevAsia: prevAsia, isFirstOdds: row == oddsList.count - 1, needBackgroundColor: row % 2 == 0)
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        headerView?.companyNameLabel.text = "\(companyInfo.name)亚盘变化"
        return headerView
    }
