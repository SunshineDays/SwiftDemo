...
/// 赛事分析 指数 大小球 详细(历史)
class FBMatchOddsBigSmallHistoryViewController: TSEmptyViewController, TSNestInnerScrollViewProtocol, FBMatchViewControllerProtocol, FBMatchOddsCompanyProtocol {
    ...
    private var oddsList = [FBOddsBigSmallModel]()
    ...
    override func getData() {
        ...
        oddsHandler.getBigSmallHistory(
            matchId: matchId,
            companyId: companyId,
            lottery: lottery,
            success: {
...
extension FBMatchOddsBigSmallHistoryViewController {
...
extension FBMatchOddsBigSmallHistoryViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let bigSmall = oddsList[row]
        let prevBigSmall = oddsList[safe: row + 1] ?? bigSmall
        cell.configCell(matchInfo: matchInfo, bigSmall: bigSmall, prevBigSmall: prevBigSmall, isFirstOdds: row == oddsList.count - 1, needBackgroundColor: row % 2 == 0)
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        headerView?.companyNameLabel.text = "\(companyInfo.name)大小球变化"
        return headerView
    }
