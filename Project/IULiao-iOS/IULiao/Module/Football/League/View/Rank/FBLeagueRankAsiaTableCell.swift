...
/// 联赛 排行榜 亚盘
class FBLeagueRankAsiaTableCell: UITableViewCell {
    ...
    func configCell(asia: FBLeagueRankAsiaModel, rank: Int) {
        rankLabel.text = "\(rank)"
        teamNameLabel.text = asia.teamName
        totalLabel.text = "\(asia.totalCount)"
        statisticsLabel.text = "\(asia.winCount)/\(asia.drawCount)/\(asia.lostCount)"
        
        for index in asia.matchResults.count..<resultLabels.count {
            let label = resultLabels[index]
            label.text = "-"
            label.backgroundColor = UIColor(hex: 0xC2C2C2)
        }
        
        for (index, result) in asia.matchResults.enumerated() {
            ...
            var color: UIColor!
            switch result {
            case .win: color = UIColor(hex: 0xE56565)
            case .draw: color = UIColor(hex: 0xE5BC65)
            case .lost: color = UIColor(hex: 0x45A1E8)
            }
