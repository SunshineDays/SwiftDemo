...
class TSRequestManager: NSObject {
    ...
    /// - Parameters:
    ///   - router: TSRouter
    ///   - expires: 缓存过期秒数 0为不缓存
    ///   - success: 成功闭包
    ///   - failed: 失败闭包
    /// - Returns: Request
    @discardableResult
    func request(router: TSRouter, expires: TimeInterval = 0, success: SuccessfulBlock? = nil, failed: FailedBlock? = nil) -> Request? {
        ...
        requestHandler = Alamofire.request(routerRequest).response {
            ...
            if let response = response {
                guard response.statusCode == 200 else {
                    ...
                    self.requestFailed(e, failed: failed)
                    return
                }
            }
            ...
            // ------ 请求失败 网络错误等
            if let error = error {
                self.requestFailed(error as NSError, failed: failed)
                return
            }
...
/*
                ...
                if expires > 0 {
                    do {
                        let cache = try Cache<NSString>(name: "request_cache")
...
*/
                ...
                let isAutoLogin = self.requestFailed(e, failed: failed)
                
                if isAutoLogin && code == kTSRequestManagerShouldLoginCode {
                    
                    UserToken.shared.clear()
                    // 发送需要登录的通知
                    NotificationCenter.default.post(name: TSNotification.userShouldLogin.notification, object: self)
                }
            }
    ...
    /// 请求错误错误处理
    @discardableResult
    private func requestFailed(_ error: NSError, failed: FailedBlock?) -> Bool{
